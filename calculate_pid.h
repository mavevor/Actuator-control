...
int force_threshold[3] = {920, 600, 600};
...
float K_p[3] = {0.001,0,0};
float K_i[3] = {0.0001,0,0};
float K_d[3] = {1,0,0};
...
void calculate_pid(void){                                                                   // cycle time is constant.
  ...
  for (int i = 0 ; i < 3; i++){
    if (PID_ON_STATE[i] == 1){
      ...
      if ((K_i[i] * i_error[i] > max_dist[i]) && K_i[i] != 0){i_error[i] = max_dist[i] / K_i[i];}      
      if ((K_i[i] * i_error[i] < min_dist[i]) && K_i[i] != 0){i_error[i] = min_dist[i] / K_i[i];}

      corrected_dist[i] = K_p[i] * p_error[i] + K_i[i] * i_error[i] + K_d[i] * d_error[i];
      if (corrected_dist[i] > max_dist[i]){corrected_dist[i] = max_dist[i];}
      if (corrected_dist[i] < min_dist[i]){corrected_dist[i] = min_dist[i];}
    }
    else{
